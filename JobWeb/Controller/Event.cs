...
namespace JobWeb
{
    ...
    public class EventController : BaseController
    {
        ...
        private async Task ProcessSocketConnection(IDictionary<string, object> wsEnv)
        {
            ...
            //handler.OnOpen();
            m_app.EventRegister(wsSendAsync);
            ...
            try
            {
                ...
                while (!wsEnv.TryGetValue("websocket.ClientCloseStatus", out status) || (int)status == 0)
                {
                    ...
                    string s = Encoding.UTF8.GetString(buffer.Array, 0, count);
                    //handler.OnMessage(s);
                    HostingEnvironment.QueueBackgroundWorkItem(cancellationToken => m_app.EventSend(s));
                }
            }
            catch (Exception ex)
            {
